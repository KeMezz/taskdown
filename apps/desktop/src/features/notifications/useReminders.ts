...
/**
 * 마감일 기반 리마인더 자동 관리
 * - 마감일이 설정되면 기본 시간에 알림 생성
 * - 마감일이 변경되면 기존 알림 삭제 후 새 알림 생성
 * - 마감일이 삭제되면 알림도 삭제
 */
export function useSyncTaskReminder() {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    mutationFn: async ({
      ...
      taskId: string;
      dueDate: Date | null;
      defaultReminderTime: string; // "HH:mm" 형식
    }): Promise<void> => {
      // 1. 기존 알림 삭제
      await db.delete(reminders).where(eq(reminders.taskId, taskId));
